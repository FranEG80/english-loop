...
const root = vi.hoisted(() => {
  ...
  return { actor, identity: { requireActor: vi.fn(async () => actor) }, practiceRunRepository: {}, attemptRepository: {}, progressRepository: {}, reviewRepository: {}, unitOfWork: {}, idGenerator: {}, clock: { nowIso: vi.fn(() => "now") }, domainEventDispatcher: {}, metrics: {}, getDatasetVersion: vi.fn(async () => "v1"), getActivityCatalog: vi.fn(), getTaxonomyCatalog: vi.fn(), practiceRunPlanner: {}, dailySessionRepository: {} };
});
...
describe("app practice actions", () => {
  ...
  it("submits an attempt and resolves feedback", async () => {
    ...
    expect(calls.feedback).toHaveBeenCalledWith(root.getActivityCatalog(root.actor), { id: "attempt-1" }, root.reviewRepository);
  });
});
