...
## Catálogo relacional y proveedores de persistencia

`DATASET/` continúa siendo la fuente editorial. `dataset:seed` valida el
contenido, calcula un checksum y publica un `CatalogRelease` mediante el
puntero singleton `CatalogPublication(id = "active")`. Los lectores Prisma
solo consultan ese puntero y nunca exponen releases `preparing` o `failed`.

Las actividades y lecciones se guardan como identidades estables más versiones
inmutables. Los intentos conservan `activityVersionId` y los recorridos usan
`PracticeRunItem`, por lo que una nueva publicación no modifica el histórico.

La selección de despliegue se controla con:

```text
CONTENT_SOURCE=dataset|database
DATABASE_PROVIDER=sqlite|d1|postgresql|mariadb
D1_TRANSPORT=binding|http
DATABASE_URL=...
D1_HTTP_URL=...
D1_HTTP_TOKEN=...
```

SQLite usa `@prisma/adapter-better-sqlite3`, PostgreSQL usa
`@prisma/adapter-pg` y MariaDB usa `@prisma/adapter-mariadb`. D1 no cae nunca
silenciosamente a SQLite: usa el cliente nativo con operaciones tipadas y
`batch()`, o su proxy HTTP autenticado con timestamp y nonce anti-replay.

Para Cloudflare, el Worker [`workers/d1-proxy.ts`](../workers/d1-proxy.ts)
recibe el binding `DB` y ejecuta las operaciones contra D1. Para Vercel, el
proceso Node crea `D1HttpClient` mediante `createD1Transport` y llama a ese
Worker usando `D1_HTTP_URL` y `D1_HTTP_TOKEN`. La fábrica solo se activa cuando
`DATABASE_PROVIDER=d1`; con cualquier otro proveedor devuelve `null` y no lee
el transporte D1.
...
D1_HTTP_TOKEN=un-secreto-compartido
```

El fichero [`wrangler.d1.example.jsonc`](../wrangler.d1.example.jsonc) declara
el binding `DB`. El token debe configurarse como secreto de Wrangler, no
publicarse en el repositorio.

### Configuración frente a constantes

Se llevan al entorno únicamente políticas operativas que pueden variar por
despliegue: duración de sesiones, caché de sesión y límites de rate limiting.
Los valores por defecto están en `server/infrastructure/config/config.ts` y
se validan como enteros positivos.

Se mantienen en código, con nombres explícitos, los contratos que no deben
cambiar accidentalmente entre entornos: tamaños de sesión (`5/10/15/20`),
etapas e intervalos de repaso, estados de publicación, identificador de la
publicación activa, versión del esquema y límites estructurales del protocolo
D1. Cambiarlos requiere revisión y pruebas, no una variable de despliegue.

El contrato de modelos se comprueba con:
...
Las migraciones se mantienen progresivas; por ejemplo, los campos de feedback
de opciones y tokens se añadieron en una migración posterior, sin modificar la
migración de catálogo ya publicada.
