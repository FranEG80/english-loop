...
/** Idempotent fixture owned by the authenticated demo experience. */
export function demoAccountStatements(database: D1DatabaseLike, passwordHash: string) {
  return [
    ...
    statement(database, "DELETE FROM Account WHERE userId = ? AND providerId = 'credential'", [DEMO_USER_ID]),
    statement(database, `INSERT INTO Account
      (id, accountId, providerId, password, createdAt, updatedAt, userId)
      VALUES (lower(hex(randomblob(16))), ?, 'credential', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?)`,
    [DEMO_USER_ID, passwordHash, DEMO_USER_ID]),
