...
describeDatabase("Prisma repository contracts on SQLite", () => {
  ...
  it("covers CRUD/upsert and user isolation across repositories", async () => {
    ...
    await prisma.activityVersion.create({ data: { id: "activity-version-integration", releaseId: release.id, activityId: "activity-1", checksum: "activity-checksum", activityTypeCode: "choice", skillFocus: "fill_blank", evaluatorStrategyCode: "single_option", levelCode: "B1", category: "grammar", topic: "topic", subtopic: "topic", difficulty: 1, instructions: "Choose", prompt: "Prompt", passage: null, explanation: "Explanation", tags: "[]", lessonIds: "[]", estimatedSeconds: 10, evaluatorData: "{}", statusCode: "published" } });
    const publishedRun = PracticeRun.create({ id: "run-published-version", userId, mode: "FOCUSED", scope: { level: "B1", taxonomyNodeId: "topic", taxonomyPath: [], descendantIds: ["topic"], requestedCount: 1 }, activityIds: ["activity-1"], activityVersionIds: ["activity-version-integration"], activitySnapshots: [{ id: "activity-1", versionId: "activity-version-integration", level: "B1", type: "choice", skillFocus: "fill_blank", category: "grammar", topic: "topic", subtopic: "topic", taxonomyNodeIds: ["topic"], difficulty: 1, instructions: "Choose", prompt: "Prompt", options: [{ id: "yes", text: "Yes", feedback: "Correct" }], lessonIds: [], tags: [], estimatedSeconds: 10, evaluator: { strategy: "single_option", correctOptionId: "yes" }, explanation: "Explanation", status: "published" }], currentIndex: 0, status: "in_progress", datasetVersion: "v1", dailySessionId: null, createdAt: "2026-08-03T00:00:00.000Z" });
    ...
    expect(JSON.parse((await prisma.practiceRunItem.findUnique({ where: { practiceRunId_position: { practiceRunId: publishedRun.id, position: 0 } } }))?.activitySnapshot ?? "null")).toMatchObject({ id: "activity-1", explanation: "Explanation", options: [{ text: "Yes" }] });
    expect((await runs.findById(publishedRun.id))?.currentActivitySnapshot).toMatchObject({ id: "activity-1", explanation: "Explanation" });
    ...
    await prisma.activityVersion.create({ data: { id: "activity-version-integration-v2", releaseId: newerRelease.id, activityId: "activity-1", checksum: "activity-checksum-v2", activityTypeCode: "choice", skillFocus: "fill_blank", evaluatorStrategyCode: "single_option", levelCode: "B1", category: "grammar", topic: "topic", subtopic: "topic", difficulty: 1, instructions: "Choose", prompt: "Prompt v2", passage: null, explanation: "Explanation v2", tags: "[]", lessonIds: "[]", estimatedSeconds: 10, evaluatorData: "{}", statusCode: "published" } });
