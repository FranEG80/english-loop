---
schemaVersion: 1.0.0
id: b1-vocabulary-leisure-sport-entertainment
title: "Leisure, sport and entertainment: sharing preferences"
level: B1
category: vocabulary
topic: b1-leisure-sport-entertainment
subtopics:
  - b1-leisure-sport-entertainment
difficulty: 2
estimatedMinutes: 23
learningObjectives:
  - "Describir aficiones, deportes, películas, libros y música."
  - "Expresar preferencias y valorar experiencias de ocio con adjetivos precisos."
prerequisites: []
frameworkRefs:
...
Hablar de ocio no consiste solo en decir `I like it`. Puedes explicar qué haces,
qué tipo de obra o deporte prefieres y por qué una experiencia te parece
`exciting`, `relaxing`, `moving` o `disappointing`.

# Objetivos

- Describir aficiones, deportes y actividades de tiempo libre.
- Hablar de películas, libros, conciertos y música.
...
el tiempo disponible; `leisure` es un término más general y algo más formal.
Para deportes, `play` va con deportes de equipo o juegos (`play tennis`, `play
football`), `go` con actividades en `-ing` (`go swimming`, `go cycling`) y `do`
con actividades o artes marciales (`do yoga`, `do karate`).

Una película puede ser `a comedy`, `a drama`, `a thriller` o `a documentary`.
Un `plot` es la trama, `a character` un personaje y `the setting` el lugar y
...
- `A concert` es una actuación musical; `a festival` suele incluir varios
  artistas o actividades.
- `The film was frightening` describe la película; `I was frightened` describe
  al espectador.

# Ejemplos

1. **I am keen on photography, especially street photography.** — Me interesa
   mucho la fotografía, especialmente la urbana.
2. **Our team played well but lost the final.** — Nuestro equipo jugó bien pero
   perdió la final.
3. **The documentary was fascinating and easy to follow.** — El documental fue
   fascinante y fácil de seguir.
4. **I felt disappointed because the concert started late.** — Me sentí
   decepcionado porque el concierto empezó tarde.
5. **The book is worth reading if you enjoy historical stories.** — El libro
   merece la pena si te gustan las historias históricas.
