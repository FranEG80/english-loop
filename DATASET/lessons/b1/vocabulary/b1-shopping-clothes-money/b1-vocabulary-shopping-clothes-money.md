---
schemaVersion: 1.0.0
id: b1-vocabulary-shopping-clothes-money
title: "Shopping, clothes and money: choosing and paying"
level: B1
category: vocabulary
topic: b1-shopping-clothes-money
subtopics:
  - b1-shopping-clothes-money
difficulty: 2
estimatedMinutes: 23
learningObjectives:
  - "Hablar de prendas, talla, ajuste, precio, pago, devolución y ahorro."
  - "Distinguir cost, price, value, afford, borrow, lend y spend."
prerequisites: []
frameworkRefs:
...
Comprar ropa o comparar precios implica describir talla y ajuste, preguntar por
el coste y resolver el pago o una devolución. Las diferencias entre `price`,
`cost`, `value` y `afford` ayudan a expresarse con precisión.

# Objetivos

- Describir prendas, materiales, talla, color y ajuste.
- Preguntar por precios y métodos de pago.
...
cuesta o el precio total de conseguirlo; `value` indica si ofrece una buena
relación con lo que pagas; `worth` expresa el valor que tiene: `The jacket is
worth £40.` Para hablar del comprador, `afford` significa poder pagar: `I can't
afford that coat.`

`Borrow` es recibir algo prestado durante un tiempo: `Can I borrow your bag?`
`Lend` es dárselo a otra persona: `Could you lend me your bag?` `Spend` se usa
...
- `The shirt fits me` habla de talla y forma; `the shirt suits me` habla de
  aspecto y estilo.
- `Borrow` va hacia el que recibe; `lend` va hacia el que presta.
- `Price` aparece en una etiqueta; `cost` se usa en la estructura `It costs...`.
- `Sale` es una rebaja o venta; `sell` es el verbo que usa la tienda.

# Ejemplos

1. **These trousers are too tight around the waist.** — Estos pantalones están
   demasiado ajustados de cintura.
2. **The blue scarf suits you better than the red one.** — La bufanda azul te
   queda mejor que la roja.
3. **I cannot afford a new laptop this month.** — No puedo permitirme un
   portátil nuevo este mes.
4. **Keep the receipt in case you need a refund.** — Guarda el recibo por si
   necesitas un reembolso.
5. **Could you lend me ten pounds until Friday?** — ¿Podrías prestarme diez
   libras hasta el viernes?
