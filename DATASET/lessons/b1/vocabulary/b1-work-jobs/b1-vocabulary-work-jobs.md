---
schemaVersion: 1.0.0
id: b1-vocabulary-work-jobs
title: "Work and jobs: roles, conditions and applications"
level: B1
category: vocabulary
topic: b1-work-jobs
subtopics:
...
estimatedMinutes: 22
learningObjectives:
  - "Describir puestos, responsabilidades, condiciones y búsqueda de empleo."
  - "Distinguir job, work, career, employer, employee, salary y wage."
prerequisites: []
frameworkRefs:
...
El vocabulario del trabajo cambia según hablemos del puesto, de la actividad,
de la trayectoria o de la persona. Esta distinción permite describir un empleo,
una oferta y las condiciones laborales con naturalidad.

# Objetivos

- Describir puestos, tareas y responsabilidades.
- Diferenciar `job`, `work` y `career`.
...
ser la actividad en general o el lugar donde trabajas: `I have a lot of work`
y `I am at work`. No suele llevar artículo cuando significa actividad. `A
career` es la trayectoria profesional que se desarrolla con el tiempo.

`An employer` es quien contrata y `an employee` es quien trabaja para esa
persona u organización. `A colleague` trabaja contigo; `a manager` dirige un
...
- `employer` contrata; `employee` es contratado. Las dos palabras se confunden
  porque comparten la raíz, pero tienen papeles opuestos.
- `salary` es sueldo, `wage` es salario o pago, a menudo por hora; `fee` es lo
  que paga un cliente por un servicio profesional.
- `colleague` es compañero de trabajo; `customer` es cliente.

# Ejemplos

1. **My sister has a part-time job at a local café.** — Mi hermana tiene un
   trabajo a tiempo parcial en una cafetería local.
2. **The advert asks for two years of experience.** — El anuncio pide dos años
   de experiencia.
3. **I am responsible for replying to customer emails.** — Soy responsable de
   responder a los correos de los clientes.
4. **The employer offers flexible hours and training.** — El empleador ofrece
   horarios flexibles y formación.
5. **He earns a monthly salary, not an hourly wage.** — Él cobra un sueldo
   mensual, no un salario por hora.
