---
schemaVersion: 1.0.0
id: b1-vocabulary-travel-transport
title: "Travel and transport: planning journeys and solving problems"
level: B1
category: vocabulary
topic: b1-travel-transport
subtopics:
  - b1-travel-transport
difficulty: 2
estimatedMinutes: 24
learningObjectives:
  - "Gestionar reservas, rutas, equipaje y problemas de transporte."
  - "Distinguir journey, trip, travel, tour, voyage y medios de transporte."
prerequisites:
...
Viajar exige vocabulario para reservar, orientarse, usar distintos transportes
y explicar incidencias. También hay que distinguir el desplazamiento de la
experiencia completa del viaje.

# Objetivos

- Reservar billetes y habitaciones y confirmar horarios.
- Describir rutas, equipaje, estaciones y aeropuertos.
...
expensive.` `A journey` es el desplazamiento de un lugar a otro, normalmente
con atención al trayecto: `The journey took three hours.` `A trip` es una visita
o experiencia concreta: `We went on a school trip.` `A tour` es un recorrido
organizado por varios lugares, mientras que `a voyage` suele ser un viaje largo,
especialmente por mar o al espacio.

En una reserva puedes `book a ticket`, `reserve a seat`, `check in` y `board a
plane`. En la llegada, `collect your luggage` y `go through customs`. `A
...
- `station` sirve para trenes y metro; `stop` suele ser una parada de autobús,
  tranvía o taxi.
- `arrive at` se usa con lugares pequeños (`the gate`, `the station`) y `arrive
  in` con ciudades o países (`London`, `Spain`).
- `road` es una vía; `route` es el camino o itinerario elegido.

# Ejemplos

1. **Our return flight leaves from gate 14 at six.** — Nuestro vuelo de vuelta
   sale de la puerta 14 a las seis.
2. **The train was delayed, so I missed my connection.** — El tren se retrasó,
   así que perdí mi conexión.
3. **We travelled by coach because it was cheaper.** — Viajamos en autocar
   porque era más barato.
4. **Please keep your passport in your hand luggage.** — Guarda el pasaporte en
   el equipaje de mano.
5. **The guide took us on a walking tour of the old town.** — El guía nos llevó
   a una visita a pie por el casco antiguo.
